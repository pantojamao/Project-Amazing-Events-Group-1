...
async function getEventsFromServer() {
    ...
    try {
        ...
        eventHome = response.response;
        generateEventCards(eventHome, ".swiper-wrapper");
...
async function filterData() {
    try {
        ...
        let url = `https://pro-talento.up.railway.app/api/amazing?name=${inputSearch}&category=${checkbox.join(',')}`;
        let response = await fetch(url);
        response = await response.json();
